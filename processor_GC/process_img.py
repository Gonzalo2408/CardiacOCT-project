...
def pre_process_image(
    ...
    mask = create_circular_mask(704, 704, center=[352, 352], radius=340)
    ...
    for n in range(img.shape[0]):
        ...
        frame_new = np.transpose(np.array(frame.resize((704, 704), Image.LANCZOS)), (2, 0, 1))
...
def post_process_image(
    ...
    all_output = np.zeros((n_frames, 704, 704)).astype('uint8')
...
def create_annotations(image, conv_fact=1, im_insize=704, bin_size=2):
    ...
    if lipid_pixels.size == 0 and cal_pixels.size == 0:
        ...
        for n in range(lipid_pixels.shape[0]):
            # dist_lipid[n, 0] = np.sqrt((lipid_pixels[n, 0]-vessel_com[0])**2+(lipid_pixels[n, 1]-vessel_com[1])**2)
            dist_lipid[n, 0] = np.degrees(
                np.arctan2((lipid_pixels[n, 0] - vessel_com[0]), (lipid_pixels[n, 1] - vessel_com[1])))
        ...
        lipid_ids = np.where(hist_count_lipid > 10)[0]
        if lipid_ids.size == 0:
            return np.zeros((im_insize, im_insize), np.uint8), np.zeros(360 // bin_size)
        ...
        overlap_lipid_tube = np.isin(lipid_ids, np.concatenate((wall_nids - 3, wall_nids + 3)))
        print(lipid_ids)
        print(np.concatenate((wall_nids - 3, wall_nids + 3)))
        if np.sum(overlap_lipid_tube) > 1:
            print(wall_nids)
            overlap_lipid_ids = lipid_ids[overlap_lipid_tube]
            print(overlap_lipid_ids)
            overlap_lipid_tube_diff = np.diff(overlap_lipid_ids)
            max_diff_id = np.argmax(overlap_lipid_tube_diff)
            if overlap_lipid_tube_diff[max_diff_id] > 1:
                #                 if np.isin(0,tube_ids) and np.isin(179,tube_ids):
                #                     print(max_diff_id)
                #                     if overlap_lipid_tube_diff[max_diff_id]>90:
                #                         added_ids = np.concatenate((np.arange(0, overlap_lipid_ids[max_diff_id]+1),np.arange(overlap_lipid_ids[max_diff_id+1],180)))
                #                     else:
                #                         added_ids = np.arange(overlap_lipid_ids[max_diff_id],overlap_lipid_ids[max_diff_id+1])
                #                     print(overlap_lipid_ids)
                #                     print(added_ids)
                #                 else:
                #                     added_ids = np.arange(overlap_lipid_ids[max_diff_id],overlap_lipid_ids[max_diff_id+1])
        ...
        for n in range(lipid_edges.size // 2):
            # lipid_angle_ids = lipid_ids[[lipid_edges[n],lipid_edges[n]+1]]
        ...
        if cal_pixels.size > 0:
            ...
            # lipid_ids = np.unique(np.concatenate((lipid_ids,cal_ids),0))

        lipid_bins = bins[lipid_ids]
        #        angle_id = np.argmin(hist_count_wall_tube[lipid_ids])
        #        angle = lipid_bins[angle_id]

        #         thin_id = (np.digitize(dist_wall[:, 1], bins)-1) == lipid_ids[angle_id]

        #         if dist_wall[thin_id, 0].size == 0:
        #             #thin_id = np.digitize(dist_lipid[:, 1], bins) == lipid_ids[angle_id]
        #             min_dist = 0
        #             max_dist = 0
        #         else:
        #             min_dist = np.min(dist_wall[thin_id, 0])
        #             if cal_pixels.size>0:
        #                 thin_id_cal = (np.digitize(dist_cal[:, 1], bins)-1) == lipid_ids[angle_id]
        ...
        # id1m,id2m = np.divmod(id_min,id2_lipid.shape[0])
