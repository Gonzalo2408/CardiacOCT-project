...
def create_annotations(image, conv_fact = 1, im_insize = 704, bin_size = 2):    
    ...
    if fat_pixels.size==0 and cal_pixels.size==0:
        #np.zeros((im_insize,im_insize), np.uint8), np.zeros(360//bin_size),
        cap_thickness = '%.0f' % 0
        lipid_arc = '%.0f' % 0
...
#             pil_image.paste(contour_image)
        ...
        img1 = ImageDraw.Draw(pil_image) 
        img1.fontmode = "L"
        fnt = ImageFont.truetype(r"Z:\grodriguez\CardiacOCT\post-processing\arial-unicode-ms.ttf",14)
...
def build_excel_frames(path_dir, segs_dir, excel_name):
    ...
    for file in segs_dir:
        ...
        cap_thickness, lipid_arc = create_annotations(seg_map_data[0])
