...
class Statistics:
    ...
    def get_data_filtered(self, test_sheet: pd.DataFrame, type_manual: str, type_ai: str,
                          missing: int = -99) -> Tuple[pd.Series, pd.Series, int, int]:
    ...
    def get_stats_file(self):
        """Obtain an Excel file with all the statistics results for a specific model,
           and save correlation and bland altman plots as png
        """

        types = {'Depth': -1, 'Arc': 180, 'Thickness': 500, 'FCT': 65, 'Lipid arc': 90}
        analysis_region = pd.DataFrame(columns=['Type', 'Model', 'FP', 'FN', 'Mean diff', 'SD',
                                                'Correlation', 'ICC(2,1)', 'Nº outliers', 'Outliers'])
        ...
        for measure_type in types.keys():
            ...
            manual, ai, fp, fn = self.get_data_filtered(self.measures_file,
                                                        '{} test set'.format(measure_type),
                                                        '{} {}'.format(measure_type, self.model_id))
            ...
            print('Saving plots')
            self.scatter_data_save_png(manual, ai, types[measure_type],
                                       '{} {}'.format(measure_type, self.model_id),
                                       '{}_{}'.format(measure_type.lower(), self.model_id.replace(' ', '_')))

            self.save_bland_altman(manual, ai,
                                   '{} {}'.format(measure_type, self.model_id),
                                   '{}_{}'.format(measure_type.lower(), self.model_id.replace(' ', '_')))
            ...
            analysis_region = analysis_region.append(pd.Series(analysis_list,
                                                               index=analysis_region.columns[:len(analysis_list)]),
                                                     ignore_index=True)
