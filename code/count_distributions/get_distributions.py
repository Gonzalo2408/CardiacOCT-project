...
class Get_Distributions:
    ...
    def __init__(self, data_path: str, output_filename: str, data_info: str):
        ...
        Args:
            data_path (str): path to the folder with the segmentations you want to count (e.g labelsTs)
    ...
    def get_patient_data(self, file: str) -> str:
        ...
        # Obtain format of pullback name
    ...
    def get_counts(self):
        ...
        """Creates Excel file with the label count,
           lipid and calcium measurements and size
           for every frame that contains annotations in the specified predicted folder.
           You can use this function either for the original dataset (i.e labelsTs or labelsTr folder)
           or the predicted segmentations (i.e in your model predictions)
        """

        counts_per_frame = pd.DataFrame(columns=['pullback', 'frame', 'background', 'lumen',
                                                 'guidewire', 'wall', 'lipid', 'calcium',
                                                 'media', 'catheter', 'sidebranch',
                                                 'rthrombus', 'wthrombus', 'dissection',
                                                 'rupture', 'lipid size', 'calcium size',
                                                 'lipid_arc', 'cap_thickness',
                                                 'calcium_depth', 'calcium_arc', 'calcium_thickness'])
        ...
        for file in os.listdir(self.data_path):
            ...
            if file.endswith('nii.gz'):
                ...
                _, _, calcium_depth, calcium_arc, calcium_thickness, _ = create_annotations_calcium(seg_map_data[0],
                                                                                                    font='mine')
                ...
                counts_per_frame = counts_per_frame.append(pd.Series(one_hot_list,
                                                           index=counts_per_frame.columns[:len(one_hot_list)]),
                                                           ignore_index=True)

        counts_per_frame.to_excel('{}.xlsx'.format(self.output_filename))

    def get_class_weights(self):
        """Obtain the class weights for the training. IMPORTANT: data_path must be the path to the labelsTr,
        otherwise you dont get the true distribution of the training set
        """
...
def main(argv):
    ...
    parser.add_argument('--output_filename', type=str, help="""File to store the results, which is an Excel file
                                                               (do not include the extenstion .xlsx)""")
    parser.add_argument('--data_info', type=str, help="""Excel file that contains the dataset info (train_test_split_v2,
                                                         without including the .xlsx)""")
