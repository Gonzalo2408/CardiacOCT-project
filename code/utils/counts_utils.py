...
def create_image_png(segmentation: np.array) -> np.array:
    """Returns the raw segmentation using a fixed color mapping

        segmentation (np.array): Array containing the raw segmentation frame
...
def merge_frames_into_pullbacks(path_predicted: str) -> dict:
    """Creates a dictionary with the pullback as key and a list of the frames with annotations as value.
       The values in the list are just the filenames of the frames in the predicted data folder.
