...
def resize_image(raw_frame: np.array, downsample: bool = True) -> np.array:
    ...
    Args:
        ...
        downsample (bool, optional): this checks for the case in which the shape is wrong but the spacing is correct
                                     (check script to understand better). Defaults to True.
...
def decode_rgb() -> np.array:
    ...
    # colormap = ListedColormap(colors=[tuple(row) for row in np.frombuffer(base64.b64decode(golden),
    #                                                                       dtype=np.ubyte).reshape((256, 3)) / 256.0])
