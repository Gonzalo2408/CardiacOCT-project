...
def count_distance_to_centre(region: np.array, centre_of_lumen: tuple,
                             dists: np.array, only_angle: bool = True) -> np.array:
    """Measures distance of each pixel in a region to the centre of the lumen.
       It can be returned either Euclidean or angle
...
def compute_arc_dices(orig_lipid: list, pred_lipid: list) -> Tuple[float, int, int, int]:
    """Obtain lipid or calcium arc DICEs, based on the idea that
       every bin in the image (180 in total, around the vessel) can contain or not lipid.
...
def create_annotations_lipid(image: np.array, font: str = 'cluster',
                             bin_size: int = 2) -> Tuple[np.array, np.array, float, float, list]:
    ...
    Args:
        ...
        bin_size (int, optional): specifies the size of the bins you want, so you get more or less.
                                  Default is 2 (meaning there are 180 bins)
    ...
    if lipid_pixels.size == 0 and calcium_pixels.size == 0:
        ...
        for n in range(lipid_edges.shape[0]//2):
            ...
            dotsize = 6
            img1.ellipse([(lipid_edge1[n, 0]-dotsize, lipid_edge1[n, 1]-dotsize),
                          (lipid_edge1[n, 0]+dotsize, lipid_edge1[n, 1]+dotsize)], fill=14, width=0)

            img1.ellipse([(lipid_edge2[n, 0]-dotsize, lipid_edge2[n, 1]-dotsize),
                          (lipid_edge2[n, 0]+dotsize, lipid_edge2[n, 1]+dotsize)], fill=14, width=0)

            img1.arc([(vessel_com[1]-30, vessel_com[0]-30), (vessel_com[1]+30, vessel_com[0]+30)],
                     start=lipid_angles[n, 0], end=lipid_angles[n, 1], fill=14, width=3)
        ...
        if font == 'mine':
            fnt = ImageFont.truetype(r"Z:\grodriguez\CardiacOCT\code\utils\arial-unicode-ms.ttf", 32)

        elif font == 'cluster':
            fnt = ImageFont.truetype("/mnt/netcache/diag/grodriguez/CardiacOCT/code/utils/arial-unicode-ms.ttf", 32)
...
def create_annotations_calcium(image: np.array, font: str = 'cluster',
                               bin_size: int = 2) -> Tuple[np.array, np.array, float, float, float, list]:
    ...
    if calcium_pixels.size == 0:
        ...
        dotsize = 3
        img1.ellipse([(max_value[0][1]-dotsize, max_value[0][0]-dotsize),
                      (max_value[0][1]+dotsize, max_value[0][0]+dotsize)], fill=13, width=0)

        img1.ellipse([(max_value[1][1]-dotsize, max_value[1][0]-dotsize),
                      (max_value[1][1]+dotsize, max_value[1][0]+dotsize)], fill=13, width=0)
        ...
        for n in range(region_edges.shape[0]//2):
            ...
            dotsize = 6
            img1.ellipse([(region_edge1[n, 0]-dotsize, region_edge1[n, 1]-dotsize),
                          (region_edge1[n, 0]+dotsize, region_edge1[n, 1]+dotsize)], fill=14, width=0)

            img1.ellipse([(region_edge2[n, 0]-dotsize, region_edge2[n, 1]-dotsize),
                          (region_edge2[n, 0]+dotsize, region_edge2[n, 1]+dotsize)], fill=14, width=0)

            img1.arc([(vessel_com[1]-30, vessel_com[0]-30), (vessel_com[1]+30, vessel_com[0]+30)],
                     start=region_angles[n, 0], end=region_angles[n, 1], fill=14, width=3)
