...
from tiger.io import checksum, path_exists, read_image, read_json, refresh_file_list, write_image, write_json
...
def copy_image(srcfile, dstfile):
    if srcfile.name.endswith('.nii.gz'):
        ...
        image, header = read_image(srcfile)
        write_image(dstfile, image, header)
...
def prepare(argv):
    ...
    print(f'[#] Converting images and masks')
    training = []
    for image_srcfile in sorted(image_srcdir.glob(image_glob_pattern)):
        ...
        copy_image(image_srcfile, image_dstfile)

        mask_dstfile = mask_dstdir / f'{caseid}.nii.gz'
        copy_image(mask_srcfile, mask_dstfile)
    ...
    print(f'[#] Writing metadata to dataset.json')
...
def plan_train(argv):
    ...
    parser.add_argument('--ignore_data_integrity_check', dest='verify_dataset_integrity', action='store_false', help='Skips default data integrity check')
    args = parser.parse_args(argv)
    ...
    with CustomizedCarbonTracker(prepdir / 'carbontracker', enabled=args.carbontracker):
        ...
        if path_exists(taskdir):
            ...
            cmd = [
                ...
                '-t', taskid,
                '-tl', os.getenv('nnUNet_def_n_proc', '1'),
                '-tf', os.getenv('nnUNet_def_n_proc', '1'),
            ]
            if args.verify_dataset_integrity:
                cmd.extend(['--verify_dataset_integrity'])
...
def _predict(args):
    ...
    cmd = [
        ...
        '-m', args.network,
        '-tr', args.trainer,
        '--num_threads_preprocessing', os.getenv('nnUNet_def_n_proc', '2'),
        '--num_threads_nifti_save', os.getenv('nnUNet_def_n_proc', '1'),
    ]
...
def init(argv):
    ...
    if args.checkout:
        print(f'Switching to "{args.checkout}" branch')
        subprocess.check_call([
    ...
    if args.threads:
        print(f'Setting size of worker pool to {args.threads}')
        os.environ['nnUNet_def_n_proc'] = str(args.threads)
